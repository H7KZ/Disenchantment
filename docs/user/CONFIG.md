...
**Enchantment state format:** Each entry is a string like `"sharpness:keep"`. The key is the Bukkit enchantment key (tab-completion is available in-game via the command). States:

| State | Behavior |
|---|---|
| `enable` | Enchantment can be removed (default — no entry needed in config for this). |
| `keep` | Enchantment stays on the source item; not transferred to the book. |
| `delete` | Enchantment is removed from the source item and destroyed; not transferred to the book. |
| `disable` | Blocks the entire disenchantment operation if this enchantment is present. |
...
**Cost formula:** Enchantments are processed in ascending level order. Starting from the base cost, each enchantment adds:

```
cost = cost + (level * (1 + multiply))
```

**Example:** `base = 10`, Sharpness V (level 5) and Fire Aspect I (level 1), `multiply = 0.25`:

```
Start:             10
+ Fire Aspect I:   10 + (1 * 1.25) = 11.25
+ Sharpness V:     11.25 + (5 * 1.25) = 17.5
Final XP cost:     17 levels
```
