...
Deep-dive documentation for engineers and LLM agents working on the Disenchantment codebase. Each file covers a specific subsystem in detail, including method signatures, data flows, and guidance for extending the plugin.
...
- NMS module names (`v1_18_R1`, `v1_20_R4`, `v1_21_R1`, `v1_21_R4`, `v1_21_R5`) correspond to Maven submodule directory names and the class name suffix of each `NMS_v*` implementation.
- Static imports: throughout the codebase `Disenchantment.plugin`, `Disenchantment.nms`, `Disenchantment.config`, `Disenchantment.localeConfig`, and `Disenchantment.logger` are imported statically and referenced without the class prefix.
