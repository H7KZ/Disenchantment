    ...
    public static void command(CommandSender s, String[] args) {
        ...
        switch (args[1]) {
            ...
            case "3": {
                ...
                s.sendMessage(CreateHelpTextComponent("/disench enchantments [ enchantment name ] [ enchantment level ] [ keep or cancel ]", "Change configuration of disabled enchantments. Keep - for keeping the enchantment on the item x Cancel - for canceling the operation of disenchanting. To enable the enchantment, run the same command."));
                s.sendMessage(CreateHelpTextComponent("/disench items", "Show current list of the disabled items."));
                s.sendMessage(CreateHelpTextComponent("/disench items [ material (item) ]", "Change the list of disabled items. To enable the item, run the same command."));
                break;
            }
            case "4": {
