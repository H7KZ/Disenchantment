...
/**
 * Handles the "disenchant:chances" subcommand for managing per-enchantment success
 * chances used by the disenchantment feature. Supports listing current chances and
 * setting an individual enchantment's chance (0.0-1.0).
 */
public class DisenchantChances {
    /**
     * The command definition for the disenchant:chances subcommand.
     */
    public static final CommandBuilder command = new CommandBuilder(
            ...
            new String[]{},
            false,
            DisenchantChances::execute,
            DisenchantChances::complete
    );

    /**
     * Executes the disenchant:chances command. With no extra arguments, lists all
    ...
    public static void execute(CommandSender s, String[] args) {
        Map<String, Double> chances = Config.Disenchantment.getEnchantmentChances();

        if (args.length == 1) {
            s.sendMessage(I18n.Commands.Chances.Disenchantment.title());

            if (chances.isEmpty()) {
                s.sendMessage("§7" + I18n.Commands.Chances.Disenchantment.empty());
                return;
            }

            chances.forEach((key, chance) -> s.sendMessage(I18n.Commands.Chances.Disenchantment.enchantment(key, String.valueOf(chance))));
        ...
        Config.Disenchantment.setEnchantmentChance(key, chance);
    ...
    public static List<String> complete(CommandSender sender, String[] args) {
        ...
        if (args.length == 2) {
            ...
            keys.addAll(Config.Disenchantment.getEnchantmentChances().keySet());
            for (String key : keys) {
