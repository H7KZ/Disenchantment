...
public class DisenchantWorlds {
    ...
    public static void execute(CommandSender sender, String[] args) {
        if (args.length >= 2 && args[1].equalsIgnoreCase("mode")) {
            if (args.length == 2) {
                sender.sendMessage(I18n.Messages.modeCurrent("disenchantment", Config.Disenchantment.getWorldsMode().name()));
                return;
            }
            ...
            Config.Disenchantment.setWorldsMode(mode);
            sender.sendMessage(I18n.Messages.modeSet("disenchantment", mode.name()));
            return;
        }
